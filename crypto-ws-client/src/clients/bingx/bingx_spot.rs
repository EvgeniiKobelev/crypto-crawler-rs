...
pub(super) const SPOT_WEBSOCKET_URL: &str = "wss://open-api-ws.bingx.com/market";

pub struct BingxSpotWSClient {
...
impl BingxSpotWSClient {
    pub async fn new(tx: Sender<String>, _proxy: Option<String>) -> BingxSpotWSClient {
        BingxSpotWSClient {
...
#[async_trait]
impl WSClient for BingxSpotWSClient {
    ...
    async fn subscribe_bbo(&self, symbols: &[String]) {
        warn!("BBO не поддерживается для BingX, подписываемся на depth вместо этого");
...
impl BingxCommandTranslator {
    fn subscription_command(channel: &str, symbol: &str) -> String {
        ...
        // Формируем команду подписки в формате BingX WebSocket API
...
impl MessageHandler for BingxMessageHandler {
    fn handle_message(&mut self, msg: &str) -> MiscMessage {
        ...
        if let Ok(obj) = serde_json::from_str::<HashMap<String, Value>>(msg) {
            ...
            if let Some(error) = obj.get("error") {
                warn!("Ошибка от BingX WebSocket: {:?}", error);
